...
//        try {
            ...
            if (!type.equals(DOCTYPE.IN.getName()) || type.equals(DOCTYPE.OUT.getName())) throw new IllegalArgumentException("Type "+type+" is unacceptable!");
...
//        }
    ...
    public StockDocument addDocument(StockDocument stockDocument){
        try {
            ...
            if (stockDocument.getDate().isBefore(LocalDateTime.of(LocalDate.now(),LocalTime.of(0, 0, 0)))) throw new ImpossibleConditionException("It is impossible to add expired date information");
            if (stockDocument.getDate().isAfter(LocalDateTime.now())) throw new ImpossibleConditionException("It is impossible to add not today date information");
