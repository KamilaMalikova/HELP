...
//        }
    ...
    private StockInventory in(StockInventory stockInventory){
      //  try {
         ...
         if (stockItemBalance == null) throw new NotFoundException("The item "+stockInventory.getProductName()+" does not exists");
...
//        }
    ...
    private void out(StockInventory stockInventory){
       // try {
            ...
            if (stockItemBalance.getInStockQty() < stockInventory.getAmount()) throw new BigValueException("There is not enough "+stockItemBalance.getName()+" in stock");
